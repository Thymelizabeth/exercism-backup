...
import Data.Text (Text, intercalate, splitOn, toUpper, unpack)
...
forthDrop (ForthState (_:xs) dict) = Right $ ForthState xs dict
...
evalText text state@(ForthState stack dict) =
  case map toUpper $ splitOn " " text of
    [""] -> Right state
    (":" : wordName : definition) ->
      case readMaybe $ unpack wordName :: Maybe Int of
    ...
    (word : rest) -> case readMaybe $ unpack word of
