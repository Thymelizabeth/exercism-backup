...
fn full_house(dice: Dice) -> u8 {
    let dice_count = dice.into_iter().map(|d| dice.into_iter().filter(|&v| v == d).count() as u8).collect::<Vec<u8>>();
    if dice_count.get(3).is_some() && dice_count.get(3).is_some() {
	dice.iter().sum()
    }
    else {
	0
