(module
  ...
  (global $is_open.CLOSED i32 (i32.const 0))
  (global $is_open.OPEN i32 (i32.const 1))
  ...
  (func (export "open") (result i32)
    (if (result i32)
      (i32.eq (global.get $is_open.CLOSED) (global.get $is_open))
      (then
        (global.set $is_open (global.get $is_open.OPEN))
        (i32.const 0))
      (else
        (i32.const -1)))
  )
  ...
  (func (export "close") (result i32)
    (if (result i32)
      ...
      (then
        (i32.const -1))
      (else
        (global.set $is_open (global.get $is_open.CLOSED))
